...
struct ChatView: View {
    ...
    var body: some View {
        ...
        .navigationTitle(chat.isGroup ? "\(chat.title)（\(chat.memberIDs.count)）" : chat.title)
        ...
        VStack(spacing: 8) {
            ...
            GlassEffectContainer(spacing: 12) {
                HStack(spacing: 10) {
                    ...
                    // Keep the collapsed glass surface alive independently of Menu.
                    // Applying glass directly to Menu lets the system temporarily replace
                    // the label during dismissal, which caused the visible glass to return
                    // a fraction late. The persistent rear surface removes that gap.
                    ZStack {
                        Color.clear
                            .frame(width: 50, height: 50)
...
struct ChatDetailView: View {
    ...
    var body: some View {
        ...
        .sheet(isPresented: $showAddMembers) { NavigationStack { AddGroupMembersView(chatID: chatID) } }
...
private struct AddGroupMembersView: View {
    ...
    private var existing: Set<String> { Set(store.chat(chatID).memberIDs) }
    private var candidates: [DemoContact] {
        store.contacts.filter { contact in
            !existing.contains(contact.id) &&
            (searchText.isEmpty || contact.name.localizedCaseInsensitiveContains(searchText) ||
             contact.subtitle.localizedCaseInsensitiveContains(searchText))
        }
    }
    ...
    var body: some View {
                ...
                Button {
                    ...
                    HStack(spacing: 12) {
                        ...
                        VStack(alignment: .leading, spacing: 2) {
                            ...
                            if contact.id.hasPrefix("friend-") && !contact.subtitle.isEmpty {
        ...
        .overlay {
            if candidates.isEmpty {
                ContentUnavailableView("没有可添加的联系人", systemImage: "person.2.slash",
                                       description: Text(searchText.isEmpty ? "现有联系人都已经在群里。" : "没有匹配的联系人。"))
            }
        }
        ...
        .toolbar {
            ...
            ToolbarItem(placement: .confirmationAction) {
                Button("完成") {
                    if store.addMembers(Array(selected), toGroup: chatID) { dismiss() }
                }
